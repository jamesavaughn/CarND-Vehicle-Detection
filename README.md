...
I largely relied on trial and error for all the parameters, adjusting one while holding all others constant. At first I did HOG on RGB and LUV colorspace. So I tried other parameters (orient = 6) and chaning the number of cells per block.  The results were not very accurate ~94%. I settled on the YCrCb color space and orient = 9, which produced an accuracy of ~99%. 
...
### Discussion on chosen window sizes and overlap...
I chose a window sizes (64, 64) with overlap of (0.5, 0.5)). 64x64 does a pretty good job for middle and background images. I scaled them 1.5 to generalize for foreground inmages. Also, when I chose 32x32 window sizes the number of windows to search slowed down the algorithm. I did not want to loop over each and every pixel of the image so I decreased the overlap from 0.75 to 0.5.
